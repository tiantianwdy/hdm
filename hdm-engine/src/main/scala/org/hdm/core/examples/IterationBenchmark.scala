...
import org.hdm.core.context.{HDMEntry, HDMSession, HDMServerContext, HDMContext}
import HDMContext._
...
class IterationBenchmark(val kIndex:Int = 0, val vIndex:Int = 1)  extends Serializable{
  ...
  val hdmSession = new AtomicReference[HDMSession]

  def init(context:String, localCores:Int = 0) = {
    val hDMContext = HDMContext.defaultContext
    hdmSession.set(new HDMSession(hDMContext))
    hdmSession.get().init(leader = context, slots = localCores)
    Thread.sleep(100)
    hdmSession.get()
  }
  ...
  def testGeneralIteration(dataPath:String, parallelism:Int = 4) = {
    ...
    val hdm = HDM(path).cached(parallelism, hdmSession.get())
    for(i <- 1 to 3){
      ...
      }.collect()(parallelism, hdmSession.get()).take(20).foreach(println(_))
  ...
  def testIterationWithAggregation(dataPath:String, parallelism:Int = 4): Unit ={
    ...
    for(i <- 1 to 3){
      ...
      }.collect()(parallelism, hdmSession.get()).take(20).toSeq
      aggregation += res.sum
...
//      aggregation += res.sum
  ...
  def testLinearRegression(dataPath:String, iterations:Int, p:Int = 4) = {
    ...
    implicit val hDMSession = hdmSession.get()
    val path = Path(dataPath)
    val kOffset = kIndex
    val vOffset = vIndex
...
//      val redFunc = (d1:Vector[Double], d2: Vector[Double]) => d1 + d2
  ...
  def testWeatherLR(dataPath: String, vectorLen: Int, iteration:Int, p:Int = 4, cached:Boolean = false)(implicit hDMEntry: HDMEntry) = {
    implicit  val parallelism = p
    val path = Path(dataPath)
    val rand = new Random(42)
