...
class BlockCoordinatorTest extends ClusterTestSuite {
  ...
  @Before
  def beforeTest(): Unit = {
    hDMEntry.init(slots = 0) // start master
  }

  @Test
  def testAddRemoveBlock(): Unit = {
    hDMEntry.startAsSlave(masterPath = "akka.tcp://masterSys@127.0.0.1:8999/user/smsMaster")
    val ddm = HDM.horizontal(appContext = appContext, hdmContext = hDMContext, text, text2) // register through hdm constrcutor
    Thread.sleep(1000)

    hDMEntry.removeBlock(ddm.children.head.id)
    Thread.sleep(1000)
  }
