...
class HDMLeaderTest extends ClusterTestSuite {
  ...
  def testForDebugging {
    hDMEntry.startAsMaster(port = 8999, conf = testMasterConf)
  ...
  def testLocalExecution() {
    ...
    hDMEntry.init(leader = "localhost", slots = 4)
    ...
    val future = wordCount.compute(1, hDMEntry)
    future onComplete {
      case Success(hdm) =>
...
//        hdm.asInstanceOf[HDM[_, _]].sample(10).foreach(println(_))
  ...
  def testHDFSExecution(): Unit = {
    ...
    val res = wordCountOpt.compute(4, hDMEntry)
    res onComplete {
      case Success(hdm) =>
        println("Job completed and received response:" + hdm)
//        hdm.asInstanceOf[HDM[_, _]].sample(10).foreach(println(_))
