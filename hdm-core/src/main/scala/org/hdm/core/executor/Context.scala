...
class HDMContext(defaultConf:Config) extends  Serializable with Logging {
  ...
  val SCHEDULING_POLICY_CLASS = Try {defaultConf.getString("hdm.scheduling.policy.class")} getOrElse ("org.hdm.core.scheduling.MinMinScheduling")
...
//    SmsSystem.addActor(HDMContext.CLUSTER_EXECUTOR_NAME, "localhost","org.hdm.core.coordinator.HDMClusterLeaderActor", slots)
  ...
  def startAsSlave(masterPath:String, port:Int = 10010, blockPort:Int = 9091, conf: Config = defaultConf, slots:Int = cores){
    ...
    SmsSystem.addActor(HDMContext.CLUSTER_EXECUTOR_NAME, "localhost","org.hdm.core.coordinator.HDMClusterWorkerActor", HDMWorkerParams(masterPath+"/"+HDMContext.CLUSTER_EXECUTOR_NAME, slots, blockContext))
  ...
  def startAsClient(masterPath:String, port:Int = 20010, blockPort:Int = 9092, conf: Config = defaultConf, localExecution:Boolean = false){
    ...
    if(localExecution){
      ...
      this.NETTY_BLOCK_SERVER_PORT = blockPort
      if(BLOCK_SERVER_INIT) HDMBlockManager.initBlockServer(this)
      SmsSystem.addActor(HDMContext.CLUSTER_EXECUTOR_NAME, "localhost","org.hdm.core.coordinator.HDMClusterWorkerActor", HDMWorkerParams(masterPath+"/"+HDMContext.CLUSTER_EXECUTOR_NAME, cores, blockContext))
    }
